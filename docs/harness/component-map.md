...
| `preflight` | G2 직후(사전) · 완료 보고 전(사후) (에이전트) | 사전: 기존 실패를 고칠지 무시할지 | 사전: 빌드·테스트가 현재 상태에서 도는가 · 사후: 테스트 실행 · 스펙·이슈 체크리스트를 **코드**와 대조 · 불변 조건 · 계약 diff · 레거시 잔존 · 문서 품질 · 리뷰어 | 없음 — 미검증 항목은 `report.md` "검증하지 못한 것" | `deny-unverified-completion.sh`가 이 스킬을 건너뛴 완료 선언을 되돌린다 |
...
|----|---------------|------|------|------|-------------------------|---------|
| `deny-dangerous-bash.sh` | `PreToolUse` · `Bash` | `tool_input.command` | force push(플래그 위치 무관·`+refspec`·`--force-with-lease` 포함) · 재귀 강제 삭제(플래그 조합 무관) · `git reset --hard` · 컨테이너 볼륨 삭제 | 차단. 알려진 오탐: 명령 문자열 전체를 보므로 `grep 'rm -rf'`도 막힘 — fail-closed 의도 | 없음 | 패턴을 늘리면 `scripts/hook-cases.txt`에 우회 케이스·정상 케이스를 먼저 |
| `deny-out-of-scope-write.sh` | `PreToolUse` · `Write\|Edit` | `tool_input.file_path` 또는 `notebook_path` | 실경로 정규화 후 `SCOPE/` 또는 `.claude/` 안인가. 저장소 밖 절대경로(세션 파일)는 통과 | 차단 | `SCOPE` = `{{작업 범위}}` — `.`이면 무동작 | 모노레포 부분 소유 프로젝트용. 케이스는 `SCOPE=sub`로 치환해 돈다 |
| `ask-open-request.sh` | `UserPromptSubmit` | `prompt` | `OPEN_PATTERNS`에 걸리고 `SMALL_FIX_PATTERNS`에 안 걸리는가 | 컨텍스트 주입만 (차단하면 프롬프트가 지워짐) | `OPEN_PATTERNS`·`SMALL_FIX_PATTERNS` (언어별) | `ask` 스킬의 트리거 표현과 같이 고친다 |
| `deny-unverified-completion.sh` | `Stop` | `transcript_path`·`stop_hook_active` | 마지막 사용자 메시지 이후 (a) 비문서 파일 Write/Edit 있음 (b) `TEST_CMD` Bash 실행 없음 (c) 마지막 텍스트에 완료 단정("완료·통과·끝났·마쳤·done·passed", 부정형·"완료 조건" 제외) | 되돌림(exit 2). 재진입·transcript 없음·`TEST_CMD` `(없음)`이면 통과 — Stop 훅만의 예외 | `TEST_CMD` = `{{테스트 명령}}` | 픽스처 `scripts/hook-fixtures/*.jsonl` 3종으로 판정. 완료 단정 정규식을 바꾸면 픽스처 문장도 확인 |

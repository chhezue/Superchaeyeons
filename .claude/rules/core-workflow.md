...
| 트랙 | 언제 | 진입 |
|------|------|------|
| **A. 기능** | 새 기능·API·엔티티·정책 변경 | DB·인증·3파일+ → `specify` → `{{스펙 저장소}}` → **승인 후** 구현 · 그 외 → `AGENTS.md` + 관련 `{{제품 범위}}` 확인 후 바로 구현 |
| **B. 감사·리팩터** | 기존 코드 품질 개선 (API 계약·비즈니스 로직 불변) | `safe-refactor` — 도메인 1개씩, 매 단계 승인 |
| **C. 버그** | 버그 리포트·`{{테스트 명령}}` 실패 | `debug` — 재현 → 원인 분리 → 최소 수정 |
...
**저장소 고유 릴리즈 사실(Milestone·심사 게이트·도메인 용어):** `{프로젝트}-release.md`를 따로 둔다 — 견본 `examples/tripfit/tripfit-release.md`
...
> **[스택 옵션: 스택 함정 메모]** 이 프로젝트가 쓰는 프레임워크의 **주 버전 함정**을 여기 한 문단으로 적는다 — 웹 예제 대다수가 다른 주 버전 기준이라 그대로 쓰면 깨지는 경우, 공식 문서 URL로 버전을 고정할 수 없는 경우 등. 없으면 이 절을 지운다.
> 채운 예: `examples/tripfit/g1-stack-trap.md`
...
- **트랙별 승인 대상:** A = 스펙(`{{스펙 저장소}}`) · B = `audit.md`의 A/B 항목 · C = 원인 가설과 수정 범위
...
- **보안·아키텍처 성격 변경:** STOP §6 대상(토큰·세션·결제·개인정보 저장 방식 등)이면 `{{현재동작 요약}}` 해당 절 갱신 확인
...
- **[스택 옵션: 스택 리뷰어]** 스택 코드를 **3파일 이상**·API·DB 범위로 고쳤으면 **스택 리뷰어 서브에이전트** — Java·Spring 팩이면 **`spring-reviewer`**([`.claude/agents/spring-reviewer.md`](../agents/spring-reviewer.md), 기준 SSOT: `spring-boot-java.md`). 트랜잭션 경계·N+1·ErrorCode/`@TripActivity` 누락처럼 **ArchUnit이 잡지 못하는** 결함이 대상이다 — 한 줄·단일 파일 수정은 대상 아님
...
- **트랙별 기록:** B 트랙은 `{{감사 로그}}/refactor-log.md`에 반영 이력 append

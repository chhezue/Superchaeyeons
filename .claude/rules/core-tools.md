...
Harness 형제: `core-guardrails.md` (⛔ STOP) · `core-gates.md` (멈추는 신호) · `core-scope.md` (priority·`[미정]`) · `core-followup.md` (후속·Defer·ERD) · `harness-map.md` (슬롯·옵션)
...
구조 SSOT는 `core-workflow.md`("사이클 — 4 트랙 × 4 게이트"). 여기는 **각 지점에서 무슨 도구를 쓰는지**만 매핑한다.
...
| **B. 감사·리팩터** | 기존 코드 품질 개선 (불변 조건 유지) | `Agent`(`Explore`/`general-purpose`, 읽기 전용)로 신선한 컨텍스트 감사 | **`safe-refactor`** 스킬 → `{{감사 로그}}` → 작업 단위 1개씩, 감사·구현 각각 승인 후 진행 |
| **C. 버그** | 버그 리포트·`{{테스트 명령}}` 실패 | 재현 → 원인 분리 → 수정 | **`debug`** 스킬 → 로컬 재현 절차 + 프로덕션 전용 재현(운영 서버 로그·DB) |
| **D. 하네스 이식** | 하네스를 붙이거나 `harness-map.md` 재동기화 | `Bash`·`Glob`·`Grep`으로 저장소 실측 (읽기 전용) | **`adopt`** 스킬 → 실측표 → 축·슬롯·플래그 제안(승인) → `harness-map.md`·`local-*.md` 채움 |
...
| **G2 승인** | `AskUserQuestion` · 채팅 확인 | 요청이 열려 있으면 **`ask`** 스킬(라운드 인터뷰) 먼저 · A=스펙 승인 · B=감사 문서 A/B 항목 승인 · C=원인 가설 승인. 새 이슈·브랜치·PR 생성은 **항상 먼저 확인** |
| **G3 검증** | **`preflight`** 스킬 · `code-review`/`simplify` 서브에이전트 | `{{테스트 명령}}` + 이슈·스펙 체크리스트 + API 변경 시 계약 diff 확인(도구는 lang 팩). 문서 50줄+ 변경·신규 문서면 **`doc-reviewer`**(기준: `doc-writing.md`), **[플래그: 스택 리뷰어]** 스택 코드 3파일+·API·DB 변경이면 lang 팩이 지정한 리뷰어 에이전트 |
...
계기·실측: `{{문서 루트}}/harness/layer2-workflow-skills.md` "호출 비용" 절
...
| PR·머지 전(요청 시) · **API·DB 등 Must Have급 변경은 커밋 전 기본 권장** | `code-review` / `simplify` — 둘 다 fresh 서브에이전트 컨텍스트에서 diff만 보고 판단 (`{{Git 컨벤션 SSOT}}`) |
